'''#!/home/mag1/atomate/atomate_env/bin/python'''
...
with open('input') as f: 
    for line in f:
        ...
        if 'structure_file' in line:
            ...
            acc = row[-1]
        elif 'MC_directory' in line:
            MC_dir = row[-1]
        elif 'logfile' in line:
            logfile = row[-1]
        elif 'remove_old_input_MC' in line:
            rm_mc_ip = row[-1] == 'True'
...
msg += '*** if you find this code useful, please cite the following papers: ***'
msg += '*** 1. https://doi.org/10.1038/s41524-020-0300-2 ***'
msg += '*** 2. https://doi.org/10.1021/acs.jpcc.9b09477 ***'
print(msg)
writeLog(msg)
msg = '*'*150
print(msg)
...
for i in range(num_struct):
    ...
    if acc == 'high':
        relx = MPRelaxSet(struct_current,user_incar_settings=relx_dict,user_kpoints_settings={'reciprocal_density':300},force_gamma=True,potcar_functional=xc)
    else:
        relx = MPRelaxSet(struct_current,user_incar_settings=relx_dict,force_gamma=True,potcar_functional=xc)
    ...
    if acc == 'high':
        stat = MPStaticSet(stat_struct,user_incar_settings=stat_dict,reciprocal_density=1000,force_gamma=True,potcar_functional=xc)
    else:
        stat = MPStaticSet(stat_struct,user_incar_settings=stat_dict,reciprocal_density=300,force_gamma=True,potcar_functional=xc)
...
if acc == 'high':
    stat = MPStaticSet.from_prev_calc(pre_path,user_incar_settings=mae_dict,reciprocal_density=1000,force_gamma=True,potcar_functional=xc)
else:
    stat = MPStaticSet.from_prev_calc(pre_path,user_incar_settings=mae_dict,reciprocal_density=300,force_gamma=True,potcar_functional=xc)
...
for axis in saxes:
    ...
    p = 0
    if acc == 'high':
        soc = MPSOCSet.from_prev_calc(path_coll,saxis=axis,nbands_factor=2,reciprocal_density=1000,force_gamma=True,potcar_functional=xc)
    else:
        soc = MPSOCSet.from_prev_calc(path_coll,saxis=axis,nbands_factor=2,reciprocal_density=300,force_gamma=True,potcar_functional=xc)
    ...
    for i in range(num_struct):
        ...
        if '_0' in config:
            ...
            if not os.path.exists(path+'/bader.dat'):
                ...
                aeccar1 = Chgcar.from_file(path+'/AECCAR0')
                msg = 'starting bader analysis for '+config
                print(msg)
                writeLog(msg)
                ba = bader_analysis_from_objects(chgcar=chgcar,potcar=potcar,aeccar0=aeccar0,aeccar2=aeccar0)
